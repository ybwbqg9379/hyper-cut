...
export const toggleBookmarkTool: AgentTool = {
	...
	description:
		"在当前播放头位置添加或移除书签。Toggle bookmark at the current playhead position.",
	parameters: {
		type: "object",
		properties: {},
		required: [],
		...
		try {
			...
			if (argName) {
				targetScene = scenes.find(
					(s) => s.name.toLowerCase() === argName.toLowerCase(),
				);
...
export const listScenesTool: AgentTool = {
	name: "list_scenes",
	description:
		"获取项目中所有场景的列表。Get a list of all scenes in the project.",
	parameters: {
		type: "object",
		properties: {},
		required: [],
...
export const renameSceneTool: AgentTool = {
	name: "rename_scene",
	description:
		"重命名当前场景或指定场景。Rename the current scene or a specific scene.",
	parameters: {
		type: "object",
		properties: {
			newName: {
			...
			sceneId: {
				type: "string",
				description:
					"Optional: ID of the scene to rename (defaults to active scene)",
			},
		},
		required: ["newName"],
	},
...
export const deleteSceneTool: AgentTool = {
	name: "delete_scene",
	description:
		"删除指定场景（按ID/名称/索引）。Delete a scene by ID, name, or index.",
	parameters: {
		type: "object",
		properties: {
			sceneId: {
