...
export const exportVideoTool: AgentTool = {
	name: "export_video",
	description:
		"导出视频（mp4/webm），支持质量与是否包含音频。Export the project to video (mp4/webm).",
	parameters: {
		type: "object",
		properties: {
			format: {
	...
	execute: async (params): Promise<ToolResult> => {
		try {
			...
			const formatParam =
				typeof params.format === "string" ? params.format : "";
			const qualityParam =
...
export const updateProjectSettingsTool: AgentTool = {
	...
	parameters: {
		...
		properties: {
			...
			canvasPreset: {
				type: "string",
				description:
					"画布尺寸预设（如 1920x1080）(Canvas preset like 1920x1080)",
			},
			background: {
				type: "object",
				properties: {
	...
	execute: async (params): Promise<ToolResult> => {
		try {
			...
			const canvasPreset =
				typeof params.canvasPreset === "string" ? params.canvasPreset : "";
			const parsedPreset = canvasPreset ? parseCanvasSize(canvasPreset) : null;
			const parsedCanvas = parsedPreset || parseCanvasSize(params.canvasSize);
				...
				if (backgroundType === "color") {
					...
					if (typeof color !== "string" || color.trim().length === 0) {
						return {
							success: false,
							message:
								"background.color 必须是非空字符串 (Invalid background color)",
					...
					const blurIntensity = (
						params.background as { blurIntensity?: unknown }
					).blurIntensity;
					if (!isFiniteNumber(blurIntensity) || blurIntensity < 0) {
						return {
							success: false,
							message:
								"background.blurIntensity 必须为非负数字 (Invalid blur intensity)",
					...
					return {
						success: false,
						message:
							"background.type 必须是 color 或 blur (Invalid background type)",
