...
export const splitAtPlayheadTool: AgentTool = {
	...
	description:
		"在当前播放头位置分割选中的视频片段。Split the selected clip(s) at the current playhead position.",
	parameters: {
		type: "object",
		properties: {},
		required: [],
...
export const splitLeftTool: AgentTool = {
	name: "split_left",
	description:
		"在播放头位置分割并删除左侧部分。Split at playhead and remove the left portion.",
	parameters: {
		type: "object",
		properties: {},
		required: [],
...
export const splitRightTool: AgentTool = {
	name: "split_right",
	description:
		"在播放头位置分割并删除右侧部分。Split at playhead and remove the right portion.",
	parameters: {
		type: "object",
		properties: {},
		required: [],
...
export const duplicateSelectedTool: AgentTool = {
	...
	description:
		"复制当前选中的片段。Duplicate the currently selected element(s).",
	parameters: {
		type: "object",
		properties: {},
		required: [],
...
export const selectElementTool: AgentTool = {
	name: "select_element",
	description:
		"通过元素ID或轨道ID设置选中元素。Select elements by element ID(s) or track ID.",
	parameters: {
		type: "object",
		properties: {
			elementId: {
			...
			trackId: {
				type: "string",
				description:
					"轨道ID（若不提供元素ID，则选中该轨道全部元素）(Track ID to select all elements)",
			},
			mode: {
				type: "string",
				enum: ["replace", "add"],
		...
		try {
			...
			if (rawElements.length > 0) {
				for (const entry of rawElements) {
					if (!entry.elementId) {
						return {
							success: false,
							message:
								"elements 参数缺少 elementId (elements is missing elementId)",
							data: { errorCode: "INVALID_PARAMS" },
						};
					}
					const ref = findElementRef(
						entry.elementId,
				...
				return {
					success: false,
					message:
						"请提供 elementId、elementIds、elements 或 trackId (Missing selection criteria)",
					data: { errorCode: "INVALID_PARAMS" },
				};
			}
			...
			if (resolvedElements.length === 0) {
				...
				return {
					success: true,
					message:
						"没有可选元素，已清空选择 (No elements to select, selection cleared)",
			...
			const nextSelection =
				mode === "add"
					? (() => {
							...
							const seen = new Set(
								existing.map((e) => `${e.trackId}:${e.elementId}`),
							);
	...
	parameters: {
		...
		try {
			...
			const trackId = editor.timeline.addTrack({
				type: type as "video" | "audio" | "text" | "sticker",
				index,
			});
...
export const removeTrackTool: AgentTool = {
	name: "remove_track",
	description:
		"删除指定轨道（不会删除主轨道）。Remove a track by ID (main track is protected).",
	parameters: {
		type: "object",
		properties: {
			trackId: {
...
export const toggleTrackMuteTool: AgentTool = {
	...
	description:
		"切换轨道静音状态（仅 audio/video 轨道）。Toggle track mute (audio/video only).",
	parameters: {
		type: "object",
		properties: {
			trackId: {
	...
	execute: async (params): Promise<ToolResult> => {
		try {
			...
			const isMuted =
				updated && canTracktHaveAudio(updated) ? updated.muted : false;
...
export const toggleTrackVisibilityTool: AgentTool = {
	...
	description:
		"切换轨道可见性（不支持 audio 轨道）。Toggle track visibility (non-audio tracks).",
	parameters: {
		type: "object",
		properties: {
			trackId: {
	...
	execute: async (params): Promise<ToolResult> => {
		try {
			...
			const updated = editor.timeline.getTrackById({ trackId });
			const isHidden =
				updated && canTrackBeHidden(updated) ? updated.hidden : false;
...
export const generateCaptionsTool: AgentTool = {
	...
	parameters: {
		...
		properties: {
			source: {
				...
				description:
					"音频来源：selection 或 timeline (Audio source: selection or timeline)",
			},
			language: {
				type: "string",
	...
	execute: async (params): Promise<ToolResult> => {
		try {
			...
			if (
				modelIdParam &&
				!TRANSCRIPTION_MODELS.some((model) => model.id === modelIdParam)
			) {
				return {
					success: false,
			...
			if (source === "selection") {
				...
				tracksToUse = allTracks
					.map((track) => {
						...
						const elements = track.elements.filter((el) =>
			...
			if (explicitTrackId) {
				...
				const existingTextTrack = allTracks.find(
...
export const updateTextStyleTool: AgentTool = {
	...
	description:
		"更新文字元素样式（内容/字体/颜色/对齐等）。Update text element style properties.",
	parameters: {
		type: "object",
		properties: {
			elementId: {
				type: "string",
				description:
					"文字元素ID（可选，默认当前选中）(Text element ID, defaults to selected)",
			},
			trackId: {
				type: "string",
	...
	execute: async (params): Promise<ToolResult> => {
		try {
			...
			if (params.content !== undefined) {
				if (!isNonEmptyString(params.content)) {
					return {
						success: false,
						message:
							"content 必须是非空字符串 (content must be a non-empty string)",
			...
			if (params.fontFamily !== undefined) {
				if (!isNonEmptyString(params.fontFamily)) {
					return {
						success: false,
						message:
							"fontFamily 必须是非空字符串 (fontFamily must be a non-empty string)",
			...
			if (params.color !== undefined) {
				if (!isNonEmptyString(params.color)) {
					return {
						success: false,
						message:
							"color 必须是非空字符串 (color must be a non-empty string)",
			...
			if (params.textAlign !== undefined) {
				if (
					!TEXT_ALIGN_VALUES.includes(
						params.textAlign as (typeof TEXT_ALIGN_VALUES)[number],
					)
				) {
					return {
						success: false,
			...
			if (params.fontWeight !== undefined) {
				if (
					!TEXT_WEIGHT_VALUES.includes(
						params.fontWeight as (typeof TEXT_WEIGHT_VALUES)[number],
					)
				) {
					return {
						success: false,
			...
			if (params.fontStyle !== undefined) {
				if (
					!TEXT_STYLE_VALUES.includes(
						params.fontStyle as (typeof TEXT_STYLE_VALUES)[number],
					)
				) {
					return {
						success: false,
			...
			if (params.textDecoration !== undefined) {
				...
				updates.textDecoration =
					params.textDecoration as TextElement["textDecoration"];
			}
...
export const moveElementTool: AgentTool = {
	name: "move_element",
	description:
		"移动元素到指定时间或轨道。Move an element to a new time or track.",
	parameters: {
		type: "object",
		properties: {
			elementId: {
			...
			targetTrackId: {
				type: "string",
				description:
					"目标轨道ID（可选，默认不变）(Target track ID, defaults to source)",
			},
			newStartTime: {
				type: "number",
		...
		try {
			...
			if (!validation.isValid) {
				return {
					success: false,
					message:
						validation.errorMessage || "元素与轨道不兼容 (Incompatible track)",
...
export const moveElementsTool: AgentTool = {
	name: "move_elements",
	description:
		"批量移动元素到指定时间。Move multiple elements to a new start time.",
	parameters: {
		type: "object",
		properties: {
			elements: {
	...
	execute: async (params): Promise<ToolResult> => {
		try {
			...
			const elements = (params.elements as Array<Record<string, unknown>>)
				.map((entry) => ({
					trackId:
						typeof entry.trackId === "string" ? entry.trackId.trim() : "",
					elementId:
						typeof entry.elementId === "string" ? entry.elementId.trim() : "",
				}))
...
export const trimElementTool: AgentTool = {
	name: "trim_element",
	description:
		"调整元素修剪点（trimStart/trimEnd）。Adjust trimStart/trimEnd for an element.",
	parameters: {
		type: "object",
		properties: {
			elementId: {
		...
		try {
			...
			const nextTrimStart =
				params.trimStart !== undefined
					? params.trimStart
					: resolved.element.trimStart;
			const nextTrimEnd =
				params.trimEnd !== undefined
					? params.trimEnd
					: resolved.element.trimEnd;
...
export const updateElementTransformTool: AgentTool = {
	...
	execute: async (params): Promise<ToolResult> => {
		try {
			...
			if (
				!("transform" in resolved.element) ||
				!("opacity" in resolved.element)
			) {
				return {
					success: false,
			...
			if (params.transform !== undefined) {
				...
				if (
					(transform.scale !== undefined &&
						!isFiniteNumber(nextTransform.scale)) ||
					(transform.rotate !== undefined &&
						!isFiniteNumber(nextTransform.rotate)) ||
					(transform.position?.x !== undefined &&
						!isFiniteNumber(nextTransform.position.x)) ||
					(transform.position?.y !== undefined &&
						!isFiniteNumber(nextTransform.position.y))
				) {
					return {
						success: false,
			...
			if (params.opacity !== undefined) {
				if (
					...
					return {
						success: false,
						message:
							"opacity 必须在 0-1 之间 (opacity must be between 0 and 1)",
	...
	parameters: {
		...
		properties: {
			elementId: {
				type: "string",
				description:
					"贴纸元素ID（可选，默认当前选中）(Sticker element ID, defaults to selected)",
			},
			trackId: {
				type: "string",
				description: "轨道ID（可选）(Optional track ID)",
		...
		try {
			...
			if (params.transform !== undefined) {
				...
				if (
					...
					(transform.rotate !== undefined &&
						!isFiniteNumber(transform.rotate)) ||
					(transform.position?.x !== undefined &&
			...
			const raw: Partial<Omit<TextElement, "type" | "id">> = {
				...
				textDecoration: params.textDecoration as
					| TextElement["textDecoration"]
					| undefined,
...
export const splitAtTimeTool: AgentTool = {
	name: "split_at_time",
	description:
		"在指定时间点分割片段。默认优先分割当前选中片段，如未选中则分割该时间点覆盖的片段；可选 selectAll=true 分割全部片段。Split clips at a specific time (seconds).",
	parameters: {
		type: "object",
		properties: {
			time: {
			...
			selectAll: {
				type: "boolean",
				description:
					"是否分割所有片段（默认 false）(Whether to split all clips, default false)",
			},
		},
		required: ["time"],
	},
