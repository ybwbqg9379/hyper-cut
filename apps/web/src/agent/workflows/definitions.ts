...
export const WORKFLOWS: Workflow[] = [
	...
	},
	{
		...
		description:
			"一键成片轻量链路：基础视觉识别、字幕生成、质量评分与导出计划。One-click lightweight pipeline.",
		scenario: "general",
		templateDescription: "默认不做激进裁剪，优先保证字幕对齐与可读性。",
		tags: ["one-click", "lightweight", "captions", "quality"],
		quality: {
			enabled: true,
			maxIterations: 1,
			targetDurationSeconds: 60,
				...
				argumentSchema: [
					{
						key: "targetDurationSeconds",
						type: "number",
						description: "质检目标时长（秒），默认与当前时长一致",
						defaultValue: 60,
						min: 1,
						max: 7200,
					},
