...
import {
	DEFAULT_SCENE_DETECTOR_HEIGHT,
	DEFAULT_SCENE_DETECTOR_WIDTH,
...
export function computeFrameDiff({
	previousFrame,
	currentFrame,
	width = DEFAULT_SCENE_DETECTOR_WIDTH,
	height = DEFAULT_SCENE_DETECTOR_HEIGHT,
}: {
...
export class SceneDetectorService {
	async detectScenes({
		...
		const finalEnd = Math.max(
			sceneStart,
			totalDuration || previousFrame.timestamp,
		);
		scenes.push({
			startTime: sceneStart,
			endTime: finalEnd,
