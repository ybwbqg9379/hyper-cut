...
describe.skipIf(SKIP_INTEGRATION)("LMStudioProvider Integration", () => {
	...
	describe("chat - basic responses", () => {
		...
		it("should handle Chinese language input", async () => {
			const response = await provider.chat({
				messages: [{ role: "user", content: '用中文说"你好"，不要说其他的。' }],
				tools: [],
			});
	...
	describe("chat - tool calling", () => {
		...
		it("should call split_at_playhead tool when asked to split", async () => {
				...
				messages: [
					{
						role: "system",
						content:
							"You are a video editor assistant. Use tools to help users edit videos.",
					},
			...
			if (response.toolCalls.length > 0) {
				...
				expect(toolNames.some((name) => name.includes("split"))).toBe(true);
			} else {
		...
		it("should call delete_selected tool when asked to delete", async () => {
			...
			if (response.toolCalls.length > 0) {
				expect(response.finishReason).toBe("tool_calls");
				const toolNames = response.toolCalls.map((tc) => tc.name);
				expect(toolNames.some((name) => name.includes("delete"))).toBe(true);
			} else {
				expect(response.content).toBeDefined();
			}
		}, 30000);
		...
		it("should pass correct arguments for seek_forward tool", async () => {
				...
				messages: [
					{
						role: "system",
						content:
							"You are a video editor assistant. Use tools to help users edit videos. When user asks to seek/skip forward, use the seek_forward tool with the number of seconds.",
					},
			...
			if (response.toolCalls.length > 0) {
				const seekCall = response.toolCalls.find(
					(tc) => tc.name === "seek_forward",
				);
				if (seekCall) {
				...
				messages: [
					{
						role: "system",
						content:
							"You must use a tool to respond. Available tools can split, delete, or get timeline info.",
					},
	...
	describe("chat - error handling", () => {
		it("should handle conversation with multiple turns", async () => {
			...
			expect(response1.content !== null || response1.toolCalls.length > 0).toBe(
				true,
			);
			...
			const response2 = await provider.chat({
				messages: [
					...
					{
						role: "assistant",
						content: response1.content ?? "I can help you edit videos.",
					},
			...
			expect(response2.content !== null || response2.toolCalls.length > 0).toBe(
				true,
			);
		}, 60000);
	});
